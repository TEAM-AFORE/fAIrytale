...
def start():
    ...
    # _create_random_npc(), npc 생성
        # name1, traits1 = _create_random_npc()
    name1 = names_male[random.randint(0, len(names_male)-1)]
    traits1 = ""
    for i in range(2):
    ...
    name2 = names_female[random.randint(0, len(names_female)-1)]
    traits2 = ""
    for j in range(2):
        personality = personalities[random.randint(0, len(personalities)-1)]
        traits2 += personality
    ...
    npcs.append(n); memories.append(m); npc1_obsv.append(f"{name1} is {traits1}")  
    obsv.append(f"{name1} is {traits1}")  
    n, m = get_agent(name2, 36, traits2, "She is talking with " + name1)
    npcs.append(n); memories.append(m); npc2_obsv.append(f"{name2} is {traits2}")
    obsv.append(f"{name2} is {traits2}")  

    
   
    # _set_interview(npc_name, npcs, pursuade)
    if pursuade : pin = f"빨간 망토가 {npc_name}을(를) 설득하여 늑대에게 복수하러 함께 가기로 했다."
    else : pin = f"빨간 망토가 설득했지만 {npc_name}(은)는 늑대에게 복수하러 가지 않기로 했다."
    o1 = f"늑대와 어떤 관계였는지, 빨간 망토와 {npc_name}의 결정에 대해 어떻게 생각하는지"
    ...
    for k in range(2):
        ...
        if k == 0:
            npc1_obsv.append(f"{name1} thought '{context}'")
            npc1_obsv.append(pin + "는 소문을 들었다"); npc1_obsv.append(f"{name1}은(는) {name2}에게 {o1} 물어보았다.");
            context1 = context
            obsv.append(f"{name1} said {context1}")
        else : 
            npc2_obsv.append(f"{name2} thought '{context}'")
            npc2_obsv.append(pin + "는 소문을 들었다")
            context2 = context
            obsv.append(f"{name2} said {context2}")
    
    print(f"{name1} said" + "소식 들었어요?" + pin + "네요.")
    obsv.append(f"{name1} said" + "소식 들었어요?" + pin + "네요.")
    query = answer_prompt(name2)
    answer = call_answer(npcs[1], query, obsv)
    answer = parse_for_obsv(answer)
    print(answer); obsv.append(f"{name2} said {answer}")

    print(f"{name1} said {context1}")
    print("당신과 늑대 사이에는 무슨 일이 있었나요?")   
    obsv.append(f"{name1} said {context1}") 
    obsv.append(f"{name1} said 당신과 늑대 사이에는 무슨 일이 있었나요?")
    query = answer_prompt(name2)
    answer = call_answer(npcs[1], query, obsv)
    answer = parse_for_obsv(answer)
    print(answer); obsv.append(f"{name2} said {answer}")
    ...
    public_dialogues = obsv

    print("늑대에 대해 어떻게 생각해요?")   
    obsv.append(f"{name1} said 늑대에 대해 어떻게 생각해요?")
    query = answer_prompt(name2)
    answer = call_answer(npcs[1], query, obsv)
    answer = parse_for_obsv(answer)
    print(answer); obsv.append(f"{name2} said {answer}")
    ...
    turns = 0
    for d in range(3): # 대화 횟수 조정
        break_dialogue = False
        for npc in npcs:
            if npc == npcs[0]:
                query = answer_prompt(name1)
                answer = call_answer(npcs[0], query, obsv)
                answer = parse_for_obsv(answer)
                obsv.append(f"{name1} said {answer}")
            else:
                query = answer_prompt(name2)
                answer = call_answer(npcs[1], query, obsv)
                answer = parse_for_obsv(answer)
                obsv.append(f"{name2} said {answer}")
            print(answer)
            public_dialogues = obsv
            """
    ...
    ## 메모리 중요성(추가)
    """
    obsv.append(f"{name1} heard {pin}")
    obsv.append(f"{name2} heard {pin}")
    obsv.append(f"{name1} said to {name2} that '{o1}'.")

    query = answer_prompt(name2) # name2가 뭐라고 답변해야 하는지?에 대한 프롬프트 문자열
    answer1 = call_answer(npcs[1], query, npc1_obsv)
    npc1_obsv.append(f"{name2} said {answer1}")
    npc2_obsv.append(f"{name2} said {answer1}")


    query = answer_prompt(name1)
    answer2 = call_answer(npcs[0], query, npc2_obsv)
    npc1_obsv.append(f"{name1} said {answer2}")
    npc2_obsv.append(f"{name1} said {answer2}")

    query = answer_prompt(name2)
    answer3 = call_answer(npcs[1], query, npc1_obsv)
    npc1_obsv.append(f"{name2} said {answer3}")
    npc2_obsv.append(f"{name2} said {answer3}")
    """
    #return {"a1" : answer1, "a2" : answer2, "a3" : answer3}
    return json.dumps({"dialogues": public_dialogues}, ensure_ascii=False)



def answer_prompt(name):
    """ 
    (f"What would {name} say? To end the conversation, write in Korean:"
            ' GOODBYE: "what to say". Otherwise to continue the conversation,'
            ' write in Korean: SAY: "what to say next"\n\n')
    """
    return (f"What would {name} say to the observation? To continue the conversation, write in Korean:"
            ' SAY: "what to say next"\n\n')

def call_answer(npc_self, query_prompt, observation):
    ...
    consumed_tokens = npc_self.llm.get_num_tokens(  # 프롬프트 토큰 수
        prompt.format(**kwargs)
    )
    kwargs[npc_self.memory.most_recent_memories_token_key] = consumed_tokens  # 특정 기억 토큰의 위치를 추적하고 기록
    return npc_self.chain(prompt=prompt).run(**kwargs).strip()

def parse_for_obsv(answer):
    return answer[(answer.find("SAY : ")+5):]
