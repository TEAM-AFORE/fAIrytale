...
def start():
    ...
    if pursuade : pin = f"빨간 망토가 {npc_name}을(를) 설득하여 늑대에게 복수하러 함께 가기로 했다는 소문을 들었다."
    else : pin = f"빨간 망토가 설득했지만 {npc_name}(은)는 늑대에게 복수하러 가지 않기로 했다는 소문을 들었다."
    o1 = f"{name1}은(는) {name2}에게 늑대와 어떤 관계였는지, 빨간 망토와 {npc_name}의 결정에 대해 어떻게 생각하는지 대화를 주고받는다."
    ...
    for k in range(2):
        ...
        if k == 0:
            ...
            npc1_obsv.append(pin); npc1_obsv.append(o1);
        else : 
            npc2_obsv.append(f"{name2} thought {context}")
            npc2_obsv.append(pin)

    query = answer_prompt(name2) # name1가 뭐라고 답변해야 하는지?에 대한 프롬프트 문자열
    answer = call_answer(npcs[0], query, npc1_obsv)

    answer = answer[(answer.find("SAY: ")+5):]
...
def answer_prompt(name):
    return (f"What would {name} say? To end the conversation, write in Korean:"
